...
echo "CREATE DATABASE $PSQL_DATABASE;" >> tmp.sql
echo "CREATE USER \"$PSQL_USERNAME\" WITH ENCRYPTED PASSWORD '$PSQL_PASSWORD';" >> tmp.sql
echo "GRANT ALL PRIVILEGES ON DATABASE \"$PSQL_DATABASE\" TO \"$PSQL_USERNAME\";" >> tmp.sql

psql -h"$PSQL_HOST" -p"$PSQL_PORT" -U"$PSQL_ROOT_USERNAME" $PSQL_ROOT_DATABASE -f tmp.sql
