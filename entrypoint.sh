...
echo "CREATE DATABASE \"SQL_DATABASE\";" > db.sql
echo "CREATE USER \"$PSQL_USERNAME\" WITH ENCRYPTED PASSWORD '$PSQL_PASSWORD';" > user.sql
echo "GRANT ALL PRIVILEGES ON DATABASE \"$PSQL_DATABASE\" TO \"$PSQL_USERNAME\";" > grant.sql

psql -h"$PSQL_HOST" -p"$PSQL_PORT" -U"$PSQL_ROOT_USERNAME" $PSQL_ROOT_DATABASE -f db.sql
psql -h"$PSQL_HOST" -p"$PSQL_PORT" -U"$PSQL_ROOT_USERNAME" $PSQL_ROOT_DATABASE -f user.sql
psql -h"$PSQL_HOST" -p"$PSQL_PORT" -U"$PSQL_ROOT_USERNAME" $PSQL_ROOT_DATABASE -f grant.sql
